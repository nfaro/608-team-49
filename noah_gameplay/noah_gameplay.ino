...
//};
  ...
  if (millis() - start_time >= times[global_index]) {
    ...
    for (int i = global_index; i < sizeof(times); i++) {
      Serial.println(i);
      if (millis() - start_time >= times[i]){
        if(notes[i] == 1){
          Note new_note;
          new_note.x_coordinate = 0;
          new_note.rect_height = 0;
          new_note.start_time = times[i];
          new_note.duration = durations[i];
          note_line[i] = new_note;
        }
        else if(notes[i] == 2){
          Note new_note;
  ...
  tft.fillRect(0, 140, 160, 1, TFT_WHITE);
  for (int i = still_on_screen; i < global_index + 1; i++){
    
    if (note_line[i].finished == 0) {
      tft.drawRect(note_line[i].x_coordinate, note_line[i].y_coordinate, 30, note_line[i].rect_height, TFT_GREEN);
      if (millis() - start_time - note_line[i].start_time >= note_line[i].duration) {
        note_line[i].finished = 1;
      } else {
        note_line[i].rect_height += 2;
      }
    }
  }
  for (int i = still_on_screen; i < global_index; i++){
      if (note_line[i].finished == 1) {
        tft.drawRect(note_line[i].x_coordinate, note_line[i].y_coordinate, 30, note_line[i].rect_height, TFT_GREEN);
        note_line[i].y_coordinate += 2;
        if (note_line[i].y_coordinate > 159) {
          still_on_screen++;
        }
      }
  }
...
//   int diff_array_length = sizeof(song_file);
//   char* int_split = strtok(song_file, "#");
//   char *arrays[5];
//   int i = 0;
//   while (int_split) {
//     arrays[i] = int_split;
//     Serial.println(int_split);
//     int_split = strtok(NULL, "#");
//     i++;
//   }
// }
